...
public class RequestManager {
    ...
    public ServerAction parse(String msg) { // bta5od string w btrag3 action swa2 kan register aw login aw...
        ...
        if (AvailableActions.GetMyGames.getString().equals(parts[0])) {
           ...
           if (AvailableActions.BackToOnline.getString().equals(parts[0])) {
               System.out.println("in parsing back to onlie");
            String id1 = parts[1];
            int id = Integer.parseInt(id1);
...
//                }
        ...
        if (action instanceof BackToOnline) {
            System.out.println("BackToOnline");
            GameHandler.inGameClients.remove(gameHandler);
    ...
    public void kickPlayerOffGame(int id) {
        System.out.println("Kicking" + id);
        GameHandler player = null;
        for (GameHandler gh : GameHandler.inGameClients) {
            if (gh.getID() == id) {
                player = gh;
                gh.getPs().println("GameEndedResponse");
            }
        }
        if(player != null){
